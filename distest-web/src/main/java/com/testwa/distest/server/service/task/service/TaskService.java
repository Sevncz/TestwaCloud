...
public class TaskService {
     ...
     * @return
     */
    public List<Task> find(TaskListForm queryForm) {
        Map<String, Object> params = buildQueryParams(queryForm);
        return taskDAO.findByFromProject(params);
    }
     ...
     * @return
     */
    public PageResult<Task> findByPage(TaskListForm pageForm) {
    ...
    public void update(TaskUpdateForm updateForm) throws NoSuchTaskException, NoSuchTestcaseException,NoSuchAppException {
        Task task = findOne(updateForm.getTaskId());
        if (task == null) {
...
//        task.setTestcaseIds(caseIds);
    ...
    public Long save(TaskNewForm form) {
        ...
        task.setExeMode(DB.ExecutorMode.Normal);
    ...
    public int delete(List<Long> entityIds) {
        return taskDAO.delete(entityIds);
    }
}
